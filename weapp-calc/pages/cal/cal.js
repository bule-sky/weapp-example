  ...
  clickButton:function(event) {
    ...
    if(id == this.data.id1) {//退格
       ...
       this.data.arr.length=0;
        ...
        var firsWord = data.substring(0,1);
        if(firsWord == '-') {
      ...
      let curid =this.data;
      if (id == curid.id4 ||id == curid.id8 ||id == curid.id12 ||id == curid.id16) {
        if(curid.lastIsOperator ==true || curid.screenData ==0 ){
          return;
        }
      }

      var clearZero = this.data.screenData;
      var data;
      if (clearZero == 0) {
        data = event.target.id;
      }else {
        data = clearZero + event.target.id;
      };
